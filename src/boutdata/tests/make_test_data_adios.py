...
def create_dump_file(*, i, tmpdir, rng, grid_info, boundaries, fieldperp_global_yind):
    """
    Create an ADIOS2 BP file mocking up a BOUT++ output file, and also return the data
    without guard cells
    ...
    with Stream(tmpdir.joinpath("BOUT.dmp." + str(i) + ".bp"), "w") as outputfile:
        outputfile.write("x", localnx)
        outputfile.write("y", localny)
        outputfile.write("z", localnz)
        ...
        for attrname, attr in expected_file_attributes.items():
            outputfile.write_attribute(attrname, attr)
